...
// コンストラクタ
//******************************************************************************
CNormal_Enemy::CNormal_Enemy(int nPriority)
{
	m_nAttackCount = 0;
}
//******************************************************************************
...
void CNormal_Enemy::Update(void)
{
	...
	if (m_nAttackCount % 60 == 0)
	{
		...
		{
			CEnemy_Traking_Bullet::Create(D3DXVECTOR3(pos.x, pos.y, pos.z), D3DXVECTOR3(0.0f, 0.0f, D3DXToRadian(180.0f)), D3DXVECTOR3(5.0f + nCount, 5.0f + nCount, 0.0f), ENEMY_TRAKING_BULLET_SIZE, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f), CBullet::TEX_TYPE_ENEMY_NORMAL);
		}
		// カウントを0にする
		m_nAttackCount = 0;
	}
