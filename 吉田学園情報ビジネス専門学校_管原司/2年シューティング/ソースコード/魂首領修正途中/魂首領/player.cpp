	...
	if (m_bUseBom == true)
	{
		...
		if (m_bBoss == true)
		{
			...
			if (m_nBomCount % 2 == 0)
			{
				...
				pSound->PlaySound(CSound::SOUND_LABEL_SE_SHOT);
				// 弾の生成
				//CBullet::Create(D3DXVECTOR3(WeponRightpos.x + 80, WeponRightpos.y, m_Mainpos.z), D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, -15.0f, 0.0f), BULLET_BOSS_BOM_SIZE_X, BULLET_BOSS_BOM_SIZE_Y, CBullet::TYPE_PLAYER, CBullet::PATTERN_TYPE_BOSS_BOM, CBullet::TEX_TYPE_BEAM, OBJTYPE_PLAYER_BULLET);
			}
			// カウントが200以上になったら
