...
// マクロ定義
//******************************************************************************
#define NOT_POSESSION_COLOR_VALUE	(D3DXCOLOR(0.3f,0.3f,0.3f,1.0f))
#define POSESSION_COLOR_VALUE		(D3DXCOLOR(1.0f,1.0f,1.0f,1.0f))
//******************************************************************************
// 静的メンバ変数
//******************************************************************************
...
HRESULT CBomUI::Load(void)
{
	...
	//テクスチャ読み込み
	D3DXCreateTextureFromFile(pDevice, "data/Texture/UI/bom3.png", &m_pTexture);
	return S_OK;
}
//******************************************************************************
// テクスチャ破棄
...
void CBomUI::SetBomUI(int mBomPossesion)
{
	...
	{
		for (int nCount = 0; nCount < POSESSION_MAX; nCount++)
		{
	...
	{
		for (int nCount = 0; nCount < POSESSION_4; nCount++)
		{
			m_apScene2D[nCount]->SetRGBA(POSESSION_COLOR_VALUE);
		}
	...
	{
		for (int nCount = 0; nCount < POSESSION_3; nCount++)
		{
			m_apScene2D[nCount]->SetRGBA(POSESSION_COLOR_VALUE);
		}
	...
	{
		for (int nCount = 0; nCount < POSESSION_2; nCount++)
		{
			m_apScene2D[nCount]->SetRGBA(POSESSION_COLOR_VALUE);
		}
	...
	{
		for (int nCount = POSESSION_NONE; nCount < MAX_BOM; nCount++)
		{
			m_apScene2D[nCount]->SetRGBA(NOT_POSESSION_COLOR_VALUE);
		}
	}
